...
type SemesterFilterProps = {
    ...
    handleSemesterFilter: (semester: string) => void;
};

export default function SemesterFilter({ selectedSemester, handleSemesterFilter }: SemesterFilterProps) {
        ...
        try {
            ...
            if (data.status) {
                const uniqueTahunAjaran = Array.from(
                    new Set(data.data.target_semesters.map((item) => item.semester))
                );
                setSemester(uniqueTahunAjaran);
            }
        } catch (err) {
        ...
        <div className="w-full sm:w-[300px]">
            <Select onValueChange={handleSemesterFilter}>
                <SelectTrigger className="w-full">
                    <SelectValue placeholder={selectedSemester || "Filter Semester"} />
                </SelectTrigger>
                <SelectContent>
